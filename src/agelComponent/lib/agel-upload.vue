...
export default {
  ...
  methods: {
    ...
    beforeUpload(file) {
      if (this.limitSize) {
        ...
        if (kb > this.limitSize) {
          this.onMessage('warning', `最大只能上传${this.limitSize}kb大小的文件`)
          return false
        }
      }
      const beforeUpload = getProp(this.$attrs, 'beforeUpload')
      if (beforeUpload) return beforeUpload(file)
      return true
    },
    onRemove(file, fileList) {
      this.$emit('input', fileList)
      const onRemove = getProp(this.$attrs, 'onRemove')
      onRemove && onRemove(file, fileList)
    },
    onSuccess(response, file, fileList) {
      // 需经过函数返回一个 {name:"xx",url:"xxx"} 结构的对象
      const onSuccess = getProp(this.$attrs, 'onSuccess')
      if (onSuccess) {
        const resFile = onSuccess(response, file, fileList)
    ...
    onChange(file, fileList) {
      const onChange = getProp(this.$attrs, 'onChange')
      if (!this.autoUpload) {
        this.$emit('input', fileList)
      }
    ...
    onPreview(file) {
      ...
      const onPreview = getProp(this.$attrs, 'onPreview')
