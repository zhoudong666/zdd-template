    ...
    <el-option value="" disabled>
      ...
      >
        <slot name="option" slot-scope="scope" v-bind="scope">
          <span class="el-tree-node__label" :style="scope.data.style" :class="scope.data.class">{{
            scope.node.label
          }}</span>
        </slot>
      </el-tree>
...
export default {
  ...
  computed: {
    ...
    nodeKey() {
      return this.lazy ? this.labelKey : getProp(this.$attrs, 'nodeKey') || this.labelKey
    },
    treeData() {
      if (this.lazy) {
        return undefined
      } else {
        return !isEmpty(this.proxyOptions) ? this.proxyOptions : this.$attrs.data || []
      }
    },
    selectValue() {
      if (this.lazy && this.checkedNodes.length === 0 && !this.currentNode) {
        ...
        if (this.multiple) {
          return this.checkedNodes.map((node) => node[this.labelKey])
        } else if (this.currentNode) {
          return this.currentNode[this.labelKey]
        } else {
          return ''
        }
  ...
  methods: {
    ...
    handleFilterNode(filterText, data) {
      ...
      const filterNodeMethod = getProp(this.$attrs, 'filterNode')
      if (filterNodeMethod) {
    ...
    handleClear() {
      ...
      if (this.multiple) {
        const nodes = this.getValueOption()
    ...
    loadNode(node, resolve) {
      ...
      this.load(node, (nodes) => {
        ...
        this.$nextTick(() => {
          this.setSelected()
          const equal = Array.isArray(this.selectValue)
    ...
    initScroll() {
      const classname = '.agel-tree-select-popper .el-scrollbar .el-select-dropdown__wrap'
      setTimeout(() => {
        this.$nextTick(() => {
          document.querySelectorAll(classname).forEach((el) => (el.scrollTop = 0))
        })
      }, 0)
    },
    ...
    getValueOption() {
      const tree = this.$refs.ref
      if (!tree) return null
      return this.multiple ? tree.getCheckedNodes(this.leafOnly, this.includeHalfChecked) : tree.getCurrentNode()
    },
    focus() {
...
.agel-tree-select-popper .el-scrollbar .el-scrollbar__view .el-select-dropdown__item {
  height: auto;
  padding: 0;
}
