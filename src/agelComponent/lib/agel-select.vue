<template>
  <el-select
    class="agel-select agel-item-loading"
      ...
      <slot name="default">
        ...
        <template v-if="isGroup">
          <el-option-group v-for="group in filterOptions" :key="group.label" v-bind="getGroupPorps(group)">
            <el-option
              v-for="(option, index) of group.options || []"
              :key="option.value"
...
export default {
  ...
  computed: {
    ...
    popperClass() {
      const className = getProp(this.$attrs, 'popperClass') || ''
    ...
    filterOptions() {
      ...
      return value === ''
        ? this.proxyOptions
        : this.handleFilterNode(JSON.parse(JSON.stringify(this.proxyOptions)), value)
    },
    filterEmptyText() {
  ...
  methods: {
    handleFilterNode(options = [], value) {
      return options.filter((data) => {
        ...
        const isMatching = String(data.label).indexOf(value) !== -1
