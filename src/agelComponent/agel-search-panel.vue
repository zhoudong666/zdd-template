  ...
  <div :class="['agel-search-panel', { 'position-right': attrs.panelPosition === 'right' }]">
    <agel-form v-model="form" :item-ignore-keys="['collapseAlive']">
      <template v-slot:append>
        ...
        <el-form-item
          v-if="attrs.searchButton !== false"
          :label-width="attrs.collapseButton ? '10px' : gutterLabelWidth"
        >
          <el-button class="agel-search-button" v-bind="attrs.searchButton" @click="emitSearch">
            {{ attrs.searchButton.text }}
          </el-button>
        </el-form-item>
        <el-form-item
          v-if="attrs.resetButton !== false"
          :label-width="attrs.collapseButton || attrs.searchButton ? '10px' : gutterLabelWidth"
        >
          <el-button class="agel-reset-button" v-bind="attrs.resetButton" @click="emitReset">
            {{ attrs.resetButton.text }}
          </el-button>
        </el-form-item>
...
export default {
  ...
  computed: {
    attrs() {
      const a = panelProps()
      const b = this.$agelFormConfig['agel-search-panel'] || {}
      const c = getIncludeAttrs(panelPropKeys, this.form)
      const searchButton = this.getButton('searchButton', a, b, c)
      const resetButton = this.getButton('resetButton', a, b, c)
    ...
    collapseAliveKeys() {
      ...
      const keys = [].concat(this.form.collapseAlive || [])
  ...
  methods: {
    ...
    getButton(name, ...arr) {
      const buttons = arr.map((v) => v[name])
      const buttonAttr = Object.assign(...buttons)
      const vif = buttons.reverse().every((v) => v !== false)
